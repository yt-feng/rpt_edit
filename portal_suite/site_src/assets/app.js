(function () {
  ...
  // The build fingerprints the child first so module-only changes invalidate this URL.
  const NEWSFEED_APP_ASSET = "assets/newsfeed-app.js?v=__NEWSFEED_APP_SHA__";
  let newsfeedAppPromise = null;

  function initNewsfeed() {
    if (!newsfeedAppPromise) {
      newsfeedAppPromise = import(new URL(NEWSFEED_APP_ASSET, document.baseURI).href)
        .then((module) => module.initNewsfeedApp({
          CONTENT_INTL_LOCALE,
          CONTENT_LOCALE,
          authHeaders,
          authUserLabel,
