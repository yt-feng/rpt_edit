(function (root) {
  const WECHAT = "MacroGate";
  const EMAIL = "econ.scroll@gmail.com";
  ...
  function hydrate(scope) {
    ...
    target.querySelectorAll("[data-kc-chinese-only]").forEach((element) => {
      element.hidden = !isChinese;
    });
    target.querySelectorAll("[data-kc-non-chinese-only]").forEach((element) => {
      element.hidden = isChinese;
    });
