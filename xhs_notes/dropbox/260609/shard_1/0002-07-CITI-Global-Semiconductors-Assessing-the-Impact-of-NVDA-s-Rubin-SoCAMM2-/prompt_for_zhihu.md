...
Assessing the Impact of NVDA's Rubin SoCAMM2 Capacity Reduction

## CITI'S TAKE

SemiAnalysis reported that Nvidia is reducing Vera Rubin's SoCAMM2 capacity by adopting 96GB modules instead of 192GB modules. While we project the current memory shortage has affected the change in SoCAMM2 capacity due to global DRAM capacity constraints, there will be no change to SoCAMM2 demand. Rather, we expect Context Memory eXtension (CMX) technology to be utilized as G3.5 memory to address increasing KV Cache demand, boosting NAND demand.

Nvidia Reducing Vera Rubin's SoCAMM2 Memory Capacity — According to SemiAnalysis (4-5 June), Nvidia will reduce the total SoCAMM2(DRAM) capacity of each Vera Rubin NVL 72 server from 55TB to 28TB, which implies nearly \~50% reduction per rack. SemiAnalysis reported that Nvidia has decided to adopt 96GB SoCAMM2 modules instead of 192GB SoCAMM2 modules, which were originally planned to be used. Within the Vera Rubin NVL72 rack, Nvidia is seen to cut its CPU-side SoCAMM2 capacity due to memory supply constraints and cost optimization, as per the report.

Implications — We estimate the memory supply shortage has affected Nvidia's possible decision to lower the SoCAMM2 capacity per Vera Rubin CPU from 1,536GB (=192GB x 8) to 768GB (=96GB X 8). Currently, memory suppliers are able to supply only 60% of total SoCAMM2 demand due to DRAM capacity constraints. Even if Nvidia cuts the SoCAMM2 content per module from 192GB to 96GB, we expect no change to SoCAMM2 demand. Additionally, we project Context Memory eXtension (CMX) technology, which complements SoCAMM2, will be utilized as G3.5 memory in order to address increasing KV Cache capacity demand. As a result, we expect CMX adoption will eventually increase NAND demand.
