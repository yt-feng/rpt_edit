...
\- Inventory balance increase has nothing to do with material price increases. Win Semi builds up the stock ahead of peak seasons as well as considering geopolitics/supply chain security.

## - Capex and capacity

\- GaN expansion is for infrastructure (base station and satellite). The expansion is ongoing and all the investments are to debottleneck in response to demand.

\- GaAs capacity is sufficient, and any expansion in future will focus on GaN and InP.

\- New equipment purchases all need qualification.

\- Unchanged capex guidance of TWD2-3bn in 2026.

\- 65% capacity utilization is based on the total fab capacity of Win Semi.

\- Capacity utilization could reach $100\%$ in the past. With demand recovery, Win Semi could prepare for more production activities (e.g. labor forces).

\- Capacity installed is maintained at the current level. Internal funding is sufficient to meet debottlenecking capex, unless Win Semi has the intention to build a new factory.

## - Profitability

\- Standalone GM is the GM of Win Foundry. Consolidated GM includes the impact from other subsidiaries.

• GM guidance is on a consolidated basis.

\- Cellular and Wi-Fi GM is below the corporate average given mass volume applications. Infrastructure GM is much higher than the corporate average, while optical is broadly on par with the corporate average.

\- Memory cost has no direct impact on Win Semi's GM, only indirect impact via end-device demand.

## • Non-operating items

\- Cannot forecast non-operating gain or loss for 3Q26. FX has not been a huge swing factor, given natural hedge (USD-denominated revenue and procurement).
...
<table><tr><td>Issuer</td><td>Ticker</td><td>Price</td><td>Price date</td><td>Stock rating</td><td>Sector rating</td><td>Disclosures</td></tr><tr><td>WIN Semiconductors</t
