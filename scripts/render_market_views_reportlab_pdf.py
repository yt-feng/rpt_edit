...
This is a lightweight fallback/companion renderer for the LaTeX source generated by
build_market_views_pdf.py. It guarantees that a .pdf is produced without waiting
for a full TeX Live install. The .tex source is still kept in the same folder.
"""
...
def build_pdf(summary_dir: Path, output_pdf: Path) -> None:
    ...
    for section in summary.get("sections", [])[:10]:
        ...
        for fig_id in (section.get("figure_ids") or [])[:2]:
            ...
            if img:
                ...
                story.append(img)
                caption = f"{fig.get('label', 'Exhibit')} - {clean_text(fig.get('context', ''))[:220]}"
                story.append(Paragraph(caption, styles["KCCaption"]))

        refs = []
        for ref_id in (section.get("references") or [])[:12]:
            report = reports.get(ref_id)
            if report:
                refs.append(f"[{ref_id}] {clean_text(report.get('title'))}")
        if refs:
            story.append(Paragraph("References: " + "; ".join(refs), styles["KCRef"]))
    ...
    doc.build(story)
    log(f"Fast PDF generated: {output_pdf}")
