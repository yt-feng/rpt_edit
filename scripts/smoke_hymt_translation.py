...
def main() -> int:
    ...
    public_model_calls: list[dict] = []
    translator = OfflineTranslator(diagnostic_callback=public_model_calls.append)
    ...
    def record(row, operation):
        ...
        row['model_calls'] = list(public_model_calls)
    ...
    for source in ('今年公司收入增长12.5%，营业利润率为8%。',
        ...
        for language in locales.LOCALES:
            ...
            def translate_locale(language=language, unit=unit, protected=protected, source=source):
                value = translator.translate(unit.source, language)
                locales.validate_translation_quality(language, unit, value)
                restored = protected.restore(value)
                issues = quantity_issues(source, restored, 'zh', language)
                if issues:
                    raise ValueError('Restored locale quantity check failed: ' + '; '.join(issues))
                return restored
    ...
    markdown = ('# Financial outlook\n\nRevenue fell by 8.2% year on year in the first half of 2026. '
                ...
                'See [[PORTAL_IMAGE_002]] for the chart.\n\n'
                'See [the appendix](../reports/appendix_(2026).pdf) for source data.\n\n'
                '```python\nrevenue = 120\n```\n')
    def translate_report():
        value = translator.translate_markdown(markdown, 'zh', 'en')
        for token in ('https://example.org/report.pdf', 'https://example.org/figure.png', '| --- | --- |', '[[PORTAL_IMAGE_001]]', '[[PORTAL_IMAGE_002]]',
                      '../reports/appendix_(2026).pdf', '```python\nrevenue = 120\n```'):
    ...
    for row in report['samples']:
        ...
        for call in row.get('model_calls', []):
            lines += ['Model input:', '', call['model_input'], '', 'Raw model output:', '', call['raw_translation'], '']
