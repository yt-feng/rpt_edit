...
assert.match(
  app,
  /loadAccountAdminSummary\(workerUrl, targets\)\.then\([\s\S]*?loadFreshAdminUsers\(workerUrl, targets\)/,
...
(async () => {
  ...
  const persistenceResult = await vm.runInNewContext(`
    ...
    const ACTIVE_STATUSES = new Set(["active", "trialing"]);
    const VID2PPT_GIFT_SOURCES = new Set(["vid2ppt_nova", "vid2ppt_atlas"]);
    const VID2PPT_KCDESK_GIFT_PLANS = { "NOVA-Q": { months: 3, label: "NOVA quarter" } };
