...
def make_mixed_video(item_dir: Path, args: argparse.Namespace, status: dict[str, Any]) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="kc_mixed_frames_") as temp_dir:
        tmp = Path(temp_dir)
...
def apply_runtime_patches() -> None:
    ...
    gen.LANGS["en"]["branded_opening"] = "Welcome to KC Desk Notes. Today we are unpacking one fresh research report and the question it raises."
