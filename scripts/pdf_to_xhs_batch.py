#!/usr/bin/env python3
"""Batch convert PDFs in this repo into Xiaohongshu notes.

Flow:
1. Find PDFs under --input-dir.
2. Submit them to MinerU with the MINER_U repo secret.
3. Download MinerU markdown/images.
4. Generate note.md with DeepSeek using DEEPSEEK_API_KEY.
5. Generate a simple 3:4 cover image from the PDF first page.
"""
...
def find_pdfs(input_dir: Path, output_dir: Path) -> list[Path]:
    ...
    for path in input_dir.rglob("*.pdf"):
        if path.is_file() and output_dir not in path.resolve().parents:
            pdfs.append(path)
    return sorted(pdfs)
...
def copy_extracted_images(result_dir: Path, assets_dir: Path, max_images: int) -> list[str]:
    ...
    for index, image in enumerate(images[:max_images], 1):
        target = assets_dir / f"mineru_image_{index:02d}{image.suffix.lower()}"
...
def build_prompt(template_path: Path, source_text: str, args: argparse.Namespace) -> str:
    ...
    if len(source_text) > args.prompt_chars:
        head_len = int(args.prompt_chars * 0.72)
        tail_len = int(args.prompt_chars * 0.22)
...
def generate_note(prompt: str, args: argparse.Namespace) -> str:
    api_key = os.getenv("DEEPSEEK_API_KEY")
    if not api_key:
        return "未检测到 DEEPSEEK_API_KEY。请复制 prompt_for_xhs.md 到 DeepSeek 中生成小红书笔记。\n"
    ...
    payload = {
        ...
        "temperature": 0.7,
        "messages": [
            {"role": "system", "content": "你是顶级中文小红书内容策划，输出必须可直接粘贴发布。"},
    ...
    data = parse_json_response(response, "DeepSeek generate note")
    try:
...
def make_cover(pdf_path: Path, cover_path: Path, title: str, subtitle: str, watermark: str) -> None:
    ...
    image = center_crop(image, (1080, 1440)).filter(ImageFilter.GaussianBlur(0.4)).convert("RGBA")
    ...
    for text, y, font, fill in [
        (title, 620, title_font, (18, 18, 18, 255)),
        (subtitle, 745, subtitle_font, (70, 70, 70, 255)),
    ]:
...
def process_pdf(pdf_path: Path, result_row: dict[str, Any], output_root: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    status["images"] = copy_extracted_images(raw_dir, assets_dir, args.max_images)

    prompt = build_prompt(Path(args.prompt_template), source_text, args)
    (item_dir / "prompt_for_xhs.md").write_text(prompt, encoding="utf-8")
    note = generate_note(prompt, args)
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Convert repo PDFs to Xiaohongshu notes.")
