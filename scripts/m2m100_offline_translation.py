...
MODEL_ID = f"{MODEL}@{REVISION}:translation-v2:ct2-{MANIFEST['quantization']}-{_MANIFEST_DIGEST}"
INSTALL_COMMAND = (
...
_PROTECTED = re.compile(
    ...
    r"|(?P<zh_date>\d{4}年\d{1,2}月\d{1,2}日)"
    r"|(?P<en_date>(?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2},?\s+\d{4})"
    r"|(?P<points>[-+−]?\d+(?:\.\d+)?\s*(?:percentage points?|个百分点))"
    r"|(?P<zh_amount>[-+−]?\d+(?:\.\d+)?\s*(?:万亿|十亿|百万|千万|亿|万|千)\s*(?:美元|欧元|英镑|人民币|港元|日元))"
...
def _number_key(value: str) -> str:
    ...
    if "." in value and "," in value:
        ...
        elif separator == "," and after == 3 and value.split(separator, 1)[0] != "0" and len(value.split(separator, 1)[0]) <= 3:
            # A lone English 1,234 is thousands grouping; a dot remains a decimal. Decimal
...
def _protected_literal(value: str, kind: str | None, target: str) -> str:
    """Render Chinese currency scales explicitly in English without asking a model.

    The whole quantity is outside inference, so 1.2 亿美元 becomes USD 120 million
    by exact arithmetic rather than relaxing the numeric-equivalence gate.
    """
    if kind in {"zh_date", "en_date"}:
        try:
            if kind == "zh_date":
                year, month, day = map(int, re.findall(r"\d+", value))
                parsed = date(year, month, day)
            else:
                parsed = datetime.strptime(value.replace(",", ""), "%B %d %Y").date()
            if target == "en":
                return parsed.isoformat()
            if target == "zh":
                return f"{parsed.year}年{parsed.month}月{parsed.day}日"
        except ValueError:
            raise M2M100TranslationError("Source contains an invalid calendar date") from None
        return value
    if kind == "points":
        number = re.match(r"[-+−]?\d+(?:\.\d+)?", value).group()
        labels = {"en": " percentage points", "zh": "个百分点", "ko": "퍼센트포인트", "ja": "パーセントポイント", "ar": " نقطة مئوية"}
        return number + labels[target]
    if kind != "zh_amount" or target != "en":
        return value
    match = re.fullmatch(r"([-+−]?\d+(?:\.\d+)?)\s*(万亿|十亿|百万|千万|亿|万|千)\s*(美元|欧元|英镑|人民币|港元|日元)", value)
    if not match:
        return value
    scales = {"千": 1000, "万": 10000, "百万": 1000000, "千万": 10000000,
              "亿": 100000000, "十亿": 1000000000, "万亿": 1000000000000}
    currencies = {"美元": "USD", "欧元": "EUR", "英镑": "GBP", "人民币": "CNY", "港元": "HKD", "日元": "JPY"}
    amount = Decimal(match[1].replace("−", "-")) * scales[match[2]]
    divisor, unit = next(((scale, label) for scale, label in ((10**12, "trillion"), (10**9, "billion"),
                         (10**6, "million"), (10**3, "thousand")) if abs(amount) >= scale), (1, ""))
    number = format(amount / divisor, "f").rstrip("0").rstrip(".") if amount / divisor % 1 else str(int(amount / divisor))
    return " ".join(part for part in (currencies[match[3]], number, unit) if part)
...
def _detect_source(text: str) -> str:
    ...
    if _HIRAGANA_KATAKANA.search(text):
        return "ja"
    chinese_characters = len(_CJK.findall(text))
    latin_words = re.findall(r"[A-Za-z]{2,}", text)
    if chinese_characters:
        # An English paragraph may quote a Chinese company name. A single Han
        # character must never classify the whole report line as already Chinese.
        if len(latin_words) >= 4 and sum(map(len, latin_words)) > chinese_characters * 2:
            return "en"
        return "zh"
    return "en"
        ...
        if any(len(tokens) > _MAX_INPUT_TOKENS for tokens in encoded):
            ...
            for text, tokens in zip(texts, encoded):
                if len(tokens) <= _MAX_INPUT_TOKENS:
                    outputs.append(self.translate_batch([text], source, target)[0])
                elif len(text) < 4:
                    raise M2M100TranslationError("A single token exceeds the M2M100 input bound")
                else:
                    pieces = split_text(text, max(2, len(text) // 2))
                    outputs.append(" ".join(self.translate_batch(pieces, source, target)))
            return outputs
...
class M2M100OfflineTranslator:
    ...
    def __init__(
        ...
        *,
        ...
        self.cache_dir = Path(cache_dir or os.environ.get("M2M100_TRANSLATION_CACHE") or os.environ.get("PORTAL_OFFLINE_TRANSLATION_CACHE", ".cache/m2m100-translation"))
    ...
    def _translate_pending(self, pending: list[_TranslationPart]) -> None:
        ...
        for (source, target), parts in groups.items():
            # A single neural call per exact source even when several catalog
            # rows / paragraphs request it before the first batch is persisted.
            same_source: dict[str, list[_TranslationPart]] = defaultdict(list)
            for part in parts:
                same_source[part.core or ""].append(part)
            parts = [matches[0] for matches in same_source.values()]
            ...
            for start in range(0, len(parts), self.batch_size):
                ...
                for part, value in zip(batch, translated):
                    ...
                    for duplicate in same_source[part.core or ""]:
