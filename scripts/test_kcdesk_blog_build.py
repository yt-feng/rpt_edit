#!/usr/bin/env python3
"""Regression checks for the KC Desk static Blog build."""
...
BUILD_SCRIPT = ROOT / "scripts" / "build_kc_desk_notes_site.py"
SPEC = importlib.util.spec_from_file_location("kcdesk_site_builder", BUILD_SCRIPT)
    ...
    def test_all_sources_cutoff_deduplication_and_static_pages(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            write_payload(drafts, "xhs_notes", "260726", "draft_payload_01.json", [
                {"title": "重复文章", "author": "KC桌面", "digest": "摘要 A", "content": shared_content},
            ...
            write_payload(drafts, "institutions", "260728", "draft_payload_01.json", [
                {"title": "重复文章", "author": "KC桌面", "digest": "摘要 A", "content": f"\n{shared_content}\n"},
            ...
            articles = builder.build_blog(output, drafts, "https://kcdesk.com", date(2026, 7, 27))
            ...
            builder.build_seo_outputs(output, catalog, "https://kcdesk.com", articles)
            ...
            self.assertIn("https://kcdesk.com/blog/", sitemap_baidu)

    def test_empty_source_still_builds_blog_landing_page(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            temp = Path(temporary)
            output = temp / "site"
            articles = builder.build_blog(output, temp / "missing", "https://kcdesk.com", date(2026, 7, 27))
