#!/usr/bin/env python3
"""Mirror KC Desk operations videos from GitHub to Jianguoyun WebDAV."""
...
def build_session(token: str = "") -> requests.Session:
    ...
    session.headers.update({"User-Agent": "kcdesk-ops-video-mirror/1.0"})
    if token:
...
def discover_kc_entertainment(github: GithubSource, dates: Iterable[date]) -> list[VideoFile]:
    repo = "yt-feng/entertain_cut"
    results: list[VideoFile] = []
    for target in dates:
        generated = target.isoformat()
        root = f"outputs/kc_entertain/{generated}"
        results.extend(
            video_from_item(video, repo, generated, "KC娱乐")
...
def discover_videos(github: GithubSource, dates: list[date]) -> list[VideoFile]:
    ...
    discovered = [
        ...
        *discover_kc_entertainment(github, dates),
...
def run(args: argparse.Namespace) -> int:
    ...
    uploaded = 0
    skipped = 0
    with tempfile.TemporaryDirectory(prefix="kcdesk-jianguoyun-") as temp_dir:
