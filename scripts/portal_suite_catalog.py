#!/usr/bin/env python3
"""Build the persistent Portal Suite catalog and mirror current PDFs to R2.
...
class R2DeleteResult(NamedTuple):
...
def r2_key_for_id(report_id: str, prefix: str) -> str:
    ...
    if not REPORT_ID_RE.fullmatch(clean_id):
        raise ValueError(f"Invalid catalog report id for R2 key: {report_id!r}")
    clean_prefix = validated_catalog_r2_prefix(prefix)
    return f"{clean_prefix}/{clean_id}.pdf"


def catalog_r2_delete_key(item: dict[str, Any], prefix: str) -> str:
    """Return an exact, namespace-safe key for catalog upload or pruning."""
    report_id = str(item.get("id") or "").strip()
    if not REPORT_ID_RE.fullmatch(report_id):
        raise ValueError(f"Refusing to prune invalid catalog report id: {report_id!r}")
    clean_prefix = validated_catalog_r2_prefix(prefix)
    stored_key = item.get("r2_key")
    if stored_key is None or stored_key == "":
        return r2_key_for_id(report_id, clean_prefix)
    if not isinstance(stored_key, str) or stored_key != stored_key.strip():
        raise ValueError(f"Unsafe persisted catalog R2 key for {report_id}: {stored_key!r}")
    stored_prefix, separator, filename = stored_key.rpartition("/")
    if not separator or filename != f"{report_id}.pdf":
        raise ValueError(f"Unsafe persisted catalog R2 key for {report_id}: {stored_key!r}")
    normalized_stored_prefix = validated_catalog_r2_prefix(stored_prefix)
    expected_key = r2_key_for_id(report_id, normalized_stored_prefix)
...
def delete_r2_objects(client: Any, bucket: str, keys: list[str]) -> R2DeleteResult:
    ...
    for start in range(0, len(unique_keys), 1000):
        ...
        try:
            ...
            failed_keys.update({key: message for key in chunk})
            continue
        errors = response.get("Errors", []) if isinstance(response, dict) else []
        malformed_error = False
        for error in errors:
            if not isinstance(error, dict) or str(error.get("Key") or "") not in chunk:
                malformed_error = True
                continue
            key = str(error["Key"])
            message = str(error.get("Code") or "Error")
            if error.get("Message"):
                message += f" ({str(error['Message'])})"
            failed_keys[key] = message
        if malformed_error:
            # An unrecognized error means success cannot be attributed safely.
            failed_keys.update({key: "Unrecognized R2 deletion response" for key in chunk})
            continue
        deleted_keys.update(key for key in chunk if key not in failed_keys)
    return R2DeleteResult(frozenset(deleted_keys), failed_keys)
...
def prune_r2_objects_for_items(
    ...
    # Validate every target before opening the R2 client. Persisted keys are
    # honored so a legitimate prefix migration still deletes the old object,
    # but a malformed or reserved key aborts the whole batch fail closed.
    clean_prefix = validated_catalog_r2_prefix(r2_prefix)
    keys = [catalog_r2_delete_key(item, clean_prefix) for item in items]
    if not keys:
        return R2DeleteResult(frozenset(), {})
    bucket = require_env("R2_BUCKET")
    client = build_r2_client()
    result = delete_r2_objects(client, bucket, keys)
    log(f"Deleted {result.deleted_count} pruned PDFs from R2")
    if result.failed_keys:
        details = "; ".join(f"{key}: {message}" for key, message in sorted(result.failed_keys.items()))
        log(f"::warning title=Portal Suite R2 cleanup incomplete::{details}")
    return result
...
def previously_archived_prune_candidates(
    ...
    Physical deletion is deliberately one successful catalog run behind the
    availability change. Thus an interrupted or partially successful R2 batch
    cannot leave the currently deployed catalog advertising a missing PDF.
    """
    old_by_id = {
    ...
    for report_id, old_item in old_by_id.items():
        ...
        if old_item.get("available") or old_item.get("r2_synced"):
            continue
        if not current_item.get("pdf_archived"):
            continue
        if current_item.get("available") or current_item.get("r2_synced"):
            continue
        # Keep the previous catalog's r2_key: it is the actual object location
        # when the configured prefix has changed between runs.
        candidates.append(dict(old_item))
    return candidates
...
def merge_catalog(
    ...
    for report_id, current_item in current.items():
        ...
        if old_item.get("r2_key") and (
            old_item.get("r2_synced")
            or old_item.get("available")
            or old_item.get("pdf_archived")
        ):
            # Existing objects stay at their persisted key across a configured
            # prefix migration; only newly mirrored reports use the new prefix.
            item["r2_key"] = old_item["r2_key"]
...
def sync_current_reports_to_r2(
    ...
    with tempfile.TemporaryDirectory(prefix="portal-suite-") as tmpdir:
        ...
        for report_id in sorted(current_ids):
            ...
            if item.get("pdf_archived"):
                ...
                continue
            if item.get("r2_synced") and not item.get("pdf_object_deleted") and not force_upload:
                item["available"] = True
...
def main() -> int:
    ...
    try:
        ...
        if pruned_items:
            ...
            log("Physical R2 deletion is deferred until a later run confirms the catalog already marks these PDFs unavailable")
        ...
        if delete_candidates:
            if args.sync_r2:
                ...
                deleted_ids = {
                    ...
                    if catalog_r2_delete_key(item, args.r2_prefix) in delete_result.deleted_keys
                }
                for item in catalog.get("items", []):
        ...
        write_github_output("r2_deleted_count", str(delete_result.deleted_count))
        write_github_output("r2_delete_failed_count", str(len(delete_result.failed_keys)))
