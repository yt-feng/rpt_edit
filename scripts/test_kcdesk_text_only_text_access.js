...
const builder = fs.readFileSync(path.join(root, "scripts/build_kc_desk_notes_site.py"), "utf8");
...
vm.runInNewContext(`
  ...
  function corsHeaders() { return { "Access-Control-Allow-Origin": "https://kcdesk.com" }; }
...
assert.match(
  api.reportTextHistoryIndexUrl({ CATALOG_URL: "https://kcdesk.com/data/catalog.json" }, archiveReport),
  /\/data\/search_index_history\/shard_2607\.json$/,
  "Text-only reads must prefer the report's monthly history shard",
);
assert.match(
  api.reportTextCurrentIndexUrl({ CATALOG_URL: "https://kcdesk.com/data/catalog.json" }, archiveReport),
...
(async () => {
  ...
  console.log("KCdesk Text-only text access tests passed.");
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});
