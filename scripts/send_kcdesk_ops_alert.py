#!/usr/bin/env python3
"""Send one signed operations alert through the KC Desk Newsfeed mail worker."""
...
DEFAULT_WORKER_BASE_URL = "https://kcdesk.com/api"
...
def normalize_worker_url(value: str) -> str:
    ...
    if base.startswith("/"):
        base = f"https://kcdesk.com{base}"
    parsed = urllib.parse.urlparse(base)
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        raise ValueError("KC Desk Worker URL must be an absolute HTTP(S) URL or a /api path")
...
def signed_headers(signing_key: str, body: bytes, timestamp: int) -> dict[str, str]:
    ...
    return {
        ...
        "X-KC-Timestamp": timestamp_text,
        "X-KC-Signature": f"sha256={signature}",
    }
...
def send_alert(
    *,
        ...
        try:
            with urllib.request.urlopen(request, timeout=timeout) as response:
                ...
                if not result.get("sent"):
                    raise RuntimeError(result.get("detail") or "KC Desk Worker did not confirm the alert")
                return result
        except urllib.error.HTTPError as error:
            detail = error.read().decode("utf-8", errors="replace")[:500]
            last_error = RuntimeError(f"KC Desk Worker returned HTTP {error.code}: {detail}")
    ...
    raise RuntimeError(f"Could not send KC Desk operations alert: {last_error}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--worker-base-url",
        default=os.environ.get("KC_DESK_WORKER_URL", DEFAULT_WORKER_BASE_URL),
    )
...
def main() -> int:
    ...
    print(f"KC Desk operations alert {status} via {result.get('provider') or 'configured provider'}.")
    return 0
