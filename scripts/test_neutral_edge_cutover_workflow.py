...
class NeutralEdgeCutoverWorkflowTests(unittest.TestCase):
    ...
    def test_incremental_scope_is_default_and_shared_by_canary_and_full_build(self):
        ...
        self.assertNotIn("scripts/run_portal_locale_backfill.py", locale)
    ...
    def test_multilingual_release_is_opt_in_cached_and_transactional(self) -> None:
        ...
        for secret in (
            ...
            self.assertNotIn(f"{secret}: ${{{{ secrets.{secret} }}}}", locale)
        self.assertNotIn("DEEPSEEK_BASE_URL", locale)
        self.assertNotIn("DEEPSEEK_MODEL", locale)
        self.assertIn("--workers 1", locale)
        self.assertIn(
            "PORTAL_MULTILINGUAL_INDEX_START_DATE: "
            "${{ vars.PORTAL_MULTILINGUAL_INDEX_START_DATE || '' }}",
        ...
        self.assertNotIn("PORTAL_MULTILINGUAL_WORKERS", locale)
        for argument in (
            "--root _neutral_site",
