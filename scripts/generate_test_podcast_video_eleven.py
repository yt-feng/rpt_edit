...
- Public-facing script text is sanitized with the same spirit as xhs_notes.
- Long subtitle turns are paged upward across the audio duration instead of being cut off.
"""
...
EN_REPLACEMENTS: list[tuple[str, str]] = [
    (r"\binvestment\s+advice\b", "research discussion"),
    (r"\bstrong\s+buy\b", "constructive view"),
    (r"\bbuy\b", "research view"),
    (r"\bsell\b", "cautious view"),
    (r"\bguaranteed\b", "not assured"),
    (r"\brisk[- ]free\b", "lower-risk"),
    (r"\binsider\b", "source material"),
    (r"\bpolitics\b", "policy environment"),
    (r"\bpolitical\b", "policy-related"),
    (r"\bgeopolitical\b", "geo-policy"),
    (r"\bgovernment\b", "policy side"),
]

BRAND_REPLACEMENTS: list[tuple[str, str]] = [
    (r"Goldman\s+Sachs|高盛", "GS"),
    (r"J\.?\s*P\.?\s*Morgan|JPMorgan|摩根大通", "JPM"),
    (r"Morgan\s+Stanley|摩根士丹利|大摩", "MS"),
    (r"Bank\s+of\s+America|BofA\s+Securities|Merrill\s+Lynch|美银|美林", "BofA"),
    (r"Citigroup|Citi\s+Research|花旗", "Citi"),
    (r"UBS|瑞银", "UBS"),
...
def sanitize_public_text(text: str, lang: str = "zh") -> str:
    ...
    protected = re.sub(r"投资", "研究", protected)
    for key, token in PROTECTED_TERMS.items():
...
def split_long_text(text: str, lang: str, max_chars: int) -> list[str]:
    ...
    if len(text) <= max_chars:
        return [text]
    sep = r"(?<=[。！？；.!?;])\s*" if lang == "zh" else r"(?<=[.!?;])\s+"
    pieces = [p.strip() for p in re.split(sep, text) if p.strip()]
    chunks: list[str] = []
    buf = ""
...
def parse_script(script: str, prefixes: tuple[str, str], lang: str) -> list[tuple[str, str]]:
    ...
    max_chars = int(LANGS[lang]["max_tts_chars"])
    for raw in script.splitlines():
        match = pattern.match(raw.strip().lstrip("-• ").strip())
        if match:
            speaker = match.group(1).upper()
            for chunk in split_long_text(match.group(2), lang, max_chars):
                if chunk:
                    rows.append((speaker, chunk))
    return ensure_branded_opening(rows, lang)
...
def ensure_branded_opening(rows: list[tuple[str, str]], lang: str) -> list[tuple[str, str]]:
    ...
    if not rows:
        return rows
    joined_first = " ".join(text for _spk, text in rows[:2])
    if str(cfg["show_name"]).lower() in joined_first.lower():
        return rows
    if lang == "zh":
        opening = "欢迎来到KC桌面。今天我们用一份最新研报，拆一个值得继续跟踪的问题。"
    else:
        opening = "Welcome to KC desk note. Today we are unpacking one fresh research report and the question it raises."
    return [(str(cfg["prefixes"][0]), opening)] + rows
...
def translate_title_to_english(title: str, source_text: str, args: argparse.Namespace) -> str:
    prompt = f"""
Translate or rewrite this video title into concise English, under 60 characters.
...
def draw_frame(image_path: Path | None, title: str, title_terms: list[str], seg: dict[str, Any], output: Path, index: int, total: int, lang: str) -> None:
    ...
    watermark_font = load_font(46, True)
    ...
    draw.rounded_rectangle((116, box_top + 44, 286 if lang == "en" else 248, box_top + 92), radius=24, fill=label_color)
    draw.text((136, box_top + 48), str(label), font=small_font, fill=(255, 255, 255))

    yy = box_top + 128
    page_text = str(seg["text"])
    page_lines = page_text.split("\n") if "\n" in page_text else wrap_text(page_text, int(cfg["line_width"]), lang)[: int(cfg["max_subtitle_lines"])]
    for line in page_lines[: int(cfg["max_subtitle_lines"])] :
        draw_highlighted_line(draw, line, list(seg.get("highlight_terms", [])), subtitle_font, yy, center=False)
        yy += 66
    if int(seg.get("page_count", 1)) > 1:
        page_tip = f"{seg.get('page_index')}/{seg.get('page_count')}"
        draw.text((900, box_top + 50), page_tip, font=small_font, fill=(220, 230, 245))
    ...
    draw.text(((SIZE[0] - (box[2] - box[0])) // 2, 1812), WATERMARK, font=watermark_font, fill=(205, 220, 238))
...
def generate_language(item_dir: Path, lang: str, source_text: str, args: argparse.Namespace) -> dict[str, Any]:
    ...
    script = sanitize_public_text(deepseek(prompt, args, temperature=0.72), lang)
    (item_dir / str(cfg["script"])).write_text(script, encoding="utf-8")
    rows = parse_script(script, cfg["prefixes"], lang)
    if not rows:
    ...
    return {
        ...
        f"podcast_{lang}_visual_timeline": cfg["visual_timeline"],
...
def main() -> int:
    ...
    source_text = trim(sanitize_public_text((item_dir / "source_mineru.md").read_text(encoding="utf-8", errors="ignore"), "zh"), args.prompt_chars)
    status_update: dict[str, Any] = {
        "podcast_video_watermark": WATERMARK,
        "podcast_tts_engine_actual": "elevenlabs",
        "subtitle_source": "same text as TTS, frame durations from measured audio; long subtitles are paged upward",
