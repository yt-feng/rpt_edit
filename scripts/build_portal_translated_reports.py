...
IMAGE_TOKEN_RE = re.compile(r"\[\[PORTAL_IMAGE_(\d{3})\]\]")
...
def copy_resolved_image_for_report(
    ...
    if suffix == ".webp":
        ...
        shutil.copy2(src, target)
    token = f"[[PORTAL_IMAGE_{image_index:03d}]]"
...
1. 第一行必须是 `# {{机构中文名}}：{{短标题}}`，标题短、清楚、事实化；优先 20-35 个中文字符，最多一个冒号。
   - 原报告标题是最高权重的命题底稿，必须保留其中定义主题的公司、产品、技术、地区和关键词；允许用原文明确支持的增长、下降、压力、盈利、创纪录、超预期、改善、回升、指引变化、数据对比或反常识事实增强点击意愿。
   ...
   - 标题禁止危机、骗局、崩盘、恐慌、失败、赢家、输家、高估、低估、抄底、稳赚、必涨、震惊、唯一真相及“不是……而是……”等煽动、对立或行动建议；不要删除正常研究事实。涉及中国相关主题时仍保留具体公司、城市、数据、时间和研究对象，只中性化确有必要的立场词。
   - 军事、国防、战争、选举、政党、制裁和地缘政治等标签不进入标题；只写可公开表达的行业、技术、运营、数据或区域研究主题，但仍须生成文章。
   - 原始标题不适合直接公开时，只改写标题；绝不能拒绝、略过或删除报告，正文仍须完整生成。
   - 标题不要把三段以上信息塞满；只保留一个事实主题，其余放在开头自然展开。禁止“研究主题与行业变化观察”“某公司业务与近期数据观察”“某行业数据与主题变化观察”等占位标题。
...
def build_styles(font: str) -> dict[str, ParagraphStyle]:
    ...
    styles.add(ParagraphStyle(name="PortalBrand", fontName=font, fontSize=18, leading=23, alignment=TA_CENTER, textColor=colors.HexColor("#0B3B75"), spaceAfter=10))
    styles.add(ParagraphStyle(name="PortalTitle", fontName=font, fontSize=20, leading=27, alignment=TA_CENTER, spaceAfter=14))
    styles.add(ParagraphStyle(name="PortalH1", fontName=font, fontSize=15, leading=21, spaceBefore=14, spaceAfter=7, textColor=colors.HexColor("#0B3B75")))
    styles.add(ParagraphStyle(name="PortalH2", fontName=font, fontSize=13, leading=19, spaceBefore=11, spaceAfter=6, textColor=colors.HexColor("#1E5A8A")))
    styles.add(ParagraphStyle(name="PortalBody", fontName=font, fontSize=10.2, leading=16, spaceAfter=6))
    styles.add(ParagraphStyle(name="PortalBullet", fontName=font, fontSize=10.2, leading=16, leftIndent=0.45 * cm, firstLineIndent=-0.25 * cm, spaceAfter=5))
    styles.add(ParagraphStyle(name="PortalCaption", fontName=font, fontSize=8.2, leading=11, alignment=TA_CENTER, textColor=colors.HexColor("#666666"), spaceAfter=8))
    styles.add(ParagraphStyle(name="PortalFootnote", fontName=font, fontSize=8, leading=11, textColor=colors.HexColor("#777777"), alignment=TA_CENTER))
    return styles
...
def flush_paragraph(buffer: list[str], story: list[Any], styles: dict[str, ParagraphStyle]) -> None:
    ...
    if text:
        story.append(Paragraph(xml_text(text), styles["PortalBody"]))
    buffer.clear()
...
def render_pdf(translated_markdown: str, figures: list[dict[str, Any]], output_pdf: Path, title_hint: str) -> None:
    ...
    story: list[Any] = [
        Paragraph(BRAND, styles["PortalBrand"]),
        Paragraph(xml_text(title), styles["PortalTitle"]),
        Spacer(1, 0.2 * cm),
    ]
    ...
    for raw in markdown.splitlines():
        ...
        if token_match:
            flush_paragraph(paragraph_buffer, story, styles)
            token = f"[[PORTAL_IMAGE_{token_match.group(1)}]]"
            ...
            if img_path:
                ...
                if flowable:
                    ...
                    story.append(Paragraph(f"图表 {image_count}", styles["PortalCaption"]))
            continue
        if line.startswith("#"):
            flush_paragraph(paragraph_buffer, story, styles)
            ...
            if level <= 1:
                if normalize_space(heading) == normalize_space(title):
                    continue
                story.append(Paragraph(xml_text(heading), styles["PortalH1"]))
            else:
                story.append(Paragraph(xml_text(heading), styles["PortalH2"]))
            continue
        if re.match(r"^[-*]\s+", line) or re.match(r"^\d+[.)]\s+", line):
            flush_paragraph(paragraph_buffer, story, styles)
            bullet = re.sub(r"^(?:[-*]|\d+[.)])\s+", "", line).strip()
            story.append(Paragraph("- " + xml_text(bullet), styles["PortalBullet"]))
            continue
...
def process_report(report_dir: Path, out_dir: Path, index: int, args: argparse.Namespace) -> dict[str, Any]:
    ...
    output_pdf = report_out_dir / f"portal_translated_report_{index:02d}.pdf"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Render Portal-branded Chinese translation PDFs from MinerU report outputs.")
    parser.add_argument("--dropbox-output-root", default="xhs_notes/dropbox")
    parser.add_argument("--date-folder", default="latest")
    parser.add_argument("--output-root", default="portal_translated_reports")
