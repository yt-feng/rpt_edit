...
**许可核验状态：** Argos 库和[模型索引仓库](https://raw.githubusercontent.com/argosopentech/argospm-index/main/README.md)明确列出 MIT/CC0；本次尚未读取以上五个模型压缩包内的许可证，索引也没有逐模型许可字段，不能据此声称已确认全部模型许可。安装过程应记录实际下载包的 SHA-256、版本、来源以及包内 LICENSE/README/metadata。对模型尾部进行小范围读取的本机请求遇到证书验证错误，未更改网络或 TLS 配置。
