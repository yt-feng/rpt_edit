...
混合 Dropbox 中已经生成的中文微信导读及优化标题，在源文、正文和标题决策指纹全部匹配时直接用于文章式 PDF，不再调用一次正文和标题编辑。图片重排、固定尾注和成功的内容修订只延续修改前已验证的绑定；旧无指纹文件、源文变化或未追踪的正文/标题改动不会自动获得复用资格。PDF 仍使用本阶段严格筛选的图表和确定性文字检查。
