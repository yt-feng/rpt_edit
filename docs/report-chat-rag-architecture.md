...
Course 链路不读取约四万行的完整会员目录，而是读取私有
`_course-directory/v1/chat-index.json`。发布任务只能从已脱敏、已按字段白名单重建的
`safe_items` 派生这个对象：所有带公开机构标签的行无条件保留，每门课程再按原始顺序
保留最多 30 个不带机构标签的中性代表条目。结果必须覆盖全部 43 门课程，并受
5,000 行、2 MiB 的双重硬上限约束；任何超限都会阻止发布。完整目录仍只供会员目录
浏览 API 使用。
...
3. 运行私有 Course 目录发布任务，同时生成并校验完整目录对象与紧凑 Chat 索引对象。
4. 部署 Portal API Worker，并确认 `DEEPSEEK_API_KEY`、模型配置、R2 binding 与目录脱敏词配置存在。
5. 发布静态站点。
6. 在线验证匿名报告 Chat 为 401、普通注册用户报告 Chat 为 200、无长期会员的 Course Chat 为 403、长期会员 Course Chat 能返回具体目录资料且响应无路径或对象键。
