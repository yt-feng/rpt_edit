...
5. 输出小红书卡片图、微信/知乎/闲鱼文案、市场观点 PDF、Portal 中文精译 PDF、待发布 ZIP 包。
6. 默认把当天所有成功完成 MinerU 解析的 Portal 中文精译稿转成微信公众号草稿箱图文，一组草稿默认 9 篇文章；发布接口权限开通后可打开自动提交发布。
...
Portal Suite / Cloudflare R2 额外需要：

| Secret / Variable | 用途 |
| --- | --- |
| `R2_ACCOUNT_ID` | R2 S3 API account id，必须通过 GitHub Secret 或 Variable 配置 |
| `R2_ACCESS_KEY_ID` | R2 S3 API access key |
| `R2_SECRET_ACCESS_KEY` | R2 S3 API secret key |
| `R2_BUCKET` | R2 bucket 名，Portal Suite workflow 已默认使用 `portal-suite-pdfs` |
| `PASSWORD_SECRET` | Worker 端密码 hash pepper |
| `CALC_KEY` | Worker 隐藏计算器 key，用于计算每篇报告的伪密码 |
| `PORTAL_DOWNLOAD_PASSWORD` | PDF 下载密码，Action 会用它和 `PASSWORD_SECRET` 生成公开 hash |
| `CLOUDFLARE_API_TOKEN` | 可选，用于 Action 自动部署 Worker |
| `CLOUDFLARE_ACCOUNT_ID` | 可选，用于 Action 自动部署 Worker |
| `PORTAL_WORKER_URL` | Pages 前端调用的 Worker URL |
| `PORTAL_PAGES_URL` | 可选，Worker 读取 catalog/password rules 的 Pages URL |
...
| `GH_REPO` | 可选（Reportify 模块），repository_dispatch 目标，如 `source/example` |
...
- 自动运行：投行、国际机构或咨询报告的上游 workflow 成功完成后触发；失败或取消的上游不会触发。
...
_market-views/items/<YYMMDD>.json
```
...
### 3.5 Portal Suite Pages

文件：`.github/workflows/portal-suite-pages.yml`

用途：生成 GitHub Pages 搜索站点 **Portal Suite**。站点展示脱敏后的 Dropbox PDF 标题，并提供标题、投行目录 txt、已存在 MinerU 正文的全文搜索；PDF 不进入 repo，也不在前端暴露 Dropbox token 或 R2 key。点击报告进入详情页后，必须输入密码，由 Cloudflare Worker 校验后从私有 R2 以 attachment 方式返回 PDF。

触发方式：

- 手动运行：Actions → **Portal Suite Pages**
- 定时运行：北京时间 09:30，cron 为 `30 1 * * *`

核心数据：

```text
portal_suite/data/catalog.json      # 长期 catalog，受 Portal Suite 8GiB PDF 总量上限约束
portal_suite/data/search_index.json # 长期全文/标题检索索引，跟随 catalog 生成并提交
portal_suite/data/archive_catalog.json # Dropbox PDF 已不在但仍保留标题/正文线索的文字归档条目
portal_suite/password_rules.json    # 全局密码组 hash 对照表和分配规则，作为备用
portal_suite/site_src/              # GitHub Pages 静态站点源码
workers/portal-suite-worker/        # Cloudflare Worker 代码
```

Pages artifact 会从这些长期数据生成前端可访问文件：

```text
_portal_suite_pages/data/catalog.json
_portal_suite_pages/data/search_index.json
```
...
如果 Dropbox 已经清理掉某个 PDF，但仓库里还出现过对应投行目录标题，构建会生成 `archive_catalog.json` 里的文字归档条目：该条目 `available=false`、`pdf_archived=true`，仍可在首页按标题/正文搜索到；进入详情页时只展示文字记录和联系 Support Contact 获取原文的提示，不提供直接 PDF 下载。
...
```text
PORTAL-<base32(hmac_sha256(PASSWORD_SECRET, "portal-suite:" + report_id)) 前 12 位，按 4-4-4 分组>
```
...
如果需要，也可以继续使用 `PORTAL_DOWNLOAD_PASSWORD` 作为全局备用密码。

#### 3.5.1 Reportify 模块（其他报告）

在 Portal Suite 搜索页，本地 catalog 结果下方新增「其他报告 · Reportify」一栏，实时检索
...
workers/portal-suite-worker/src/index.js     # /reportify/search、/reportify/pdf、/reportify/status
```

额外配置：Worker 需要 `GH_REPO`（如 `source/example`）和 secret `GH_DISPATCH_TOKEN`
...
### 3.6 Portal translated reports and WeChat publishing

相关文件：

```text
.github/workflows/portal-translated-reports-test.yml
.github/workflows/portal-translated-wechat-draft-test.yml
scripts/build_portal_translated_reports.py
scripts/push_portal_translated_to_wechat_drafts.py
```

用途：

- 从 `xhs_notes/dropbox/<日期>/.../source_mineru.md` 选取报告，清理原报告 logo、作者、页脚免责声明和披露段落。
- 用 DeepSeek 翻译成中文正文，保留正文图表，渲染为带 **Portal Suite** 品牌的 PDF。
- 中文精译 PDF 每页在页眉和页脚居中放置蓝色 `Portal Suite` 标识，不再使用页面中央大水印。
- 默认把 `portal_translated_reports/<日期>/<报告>/translated.md` 转成微信公众号短版图文 HTML，可见正文约 1200 字；正文图片先走微信 `uploadimg`，封面走永久图片素材，并调用 `draft/add` 创建草稿；当 `wechat_freepublish=true` 时再调用 `freepublish/submit` 提交发布。
...
- `.github/workflows/dropbox-latest-pdf-to-xhs-sharded.yml` 的 `translated_report_count` 默认 `all`，表示当天所有成功完成 MinerU 解析并产出 `source_mineru.md` 的报告都会生成 Portal 中文精译版。手动运行时设为正整数可限制数量，设为 `0` 可关闭翻译和草稿上传。
...
- 只测翻译 PDF：Actions → **Portal translated reports PDF test**。
- 只测公众号 payload：Actions → **Portal translated WeChat draft test**，默认 `dry_run=true`，只生成 `wechat_drafts/<日期>/draft_payload_*.json` 和 summary，不调用微信 API。
- 确认 secret 和 IP 白名单可用后，把 `dry_run=false` 才会真正写入公众号草稿箱；同时把 `publish=true` 才会提交发布。

输出目录：

```text
portal_translated_reports/<日期>/<报告>/translated.md
portal_translated_reports/<日期>/<报告>/portal_translated_report_XX.pdf
...
MinerU → Portal 中文精译 → 微信公众号草稿 链路，把它们也加工成公众号图文。原始 PDF
...
IMF/WB/BIS/OECD/ADB/WEF/UNCTAD/WTO/Bruegel feeds
  ...
  -> portal_translated_reports/institutions/<日期>/<报告>/portal_translated_report_XX.pdf
...
用途：和 §3.8 并行，每天额外抓取 MBB 战略咨询的最新报告 PDF，复用 MinerU → Portal 精译 → 公众号草稿 链路。MBB 官网都是 JS + 反爬（Bain 直接 Cloudflare 挑战），所以不直接爬官网：
...
portal_translated_reports/consulting/<日期>/...
...
- `xhs_notes/consulting/<日期>` 和 `portal_translated_reports/consulting/<日期>` 是 GitHub-hosted build job 到固定 IP runner 的关键交接；推送 8 次仍失败时 workflow 必须失败，不能继续显示绿色并上传旧数据。
...
| `scripts/build_portal_translated_reports.py` | 从 MinerU 结果生成 Portal 中文精译 Markdown 和 PDF |
| `scripts/push_portal_translated_to_wechat_drafts.py` | 把 Portal 中文精译 Markdown 转成 2000 字左右的公众号草稿图文，可选 Pollinations 补图和提交发布，支持 dry-run |
...
| `scripts/portal_suite_catalog.py` | 扫描 Dropbox PDF、合并长期 catalog、按 8GiB 总量上限清理旧日期、同步当前 PDF 到 R2 |
| `scripts/build_portal_suite_site.py` | 生成 Portal Suite GitHub Pages 静态站点 artifact，并把投行目录 txt 与可匹配的 MinerU 正文并入前端全文搜索索引 |
| `scripts/hash_portal_suite_password.py` | 生成 `password_rules.json` 里的密码 hash |
...
portal_translated_reports/
portal_translated_reports/institutions/
wechat_drafts/
...
- Portal Suite 的 `portal_suite/data/catalog.json` 不按最近 3 天清理；它按页面可见 PDF 总容量控制，默认超过 8GiB 后从最旧 `date_folder` 开始删除整日旧报告，并同步删除对应 R2 object。
- Portal Suite 的 `portal_suite/data/search_index.json` 和 `portal_suite/data/archive_catalog.json` 也不按最近 3 天清理；它们用于保留历史文本检索能力。`search_index.json` 受构建参数 `--search-index-limit-gb` 控制，默认最多 2GiB，超出后按旧日期从文本索引中移除。
...
5. 如果要更新 PDF 搜索站点，跑：**Portal Suite Pages**。
