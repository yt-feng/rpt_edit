...
维护范围：`kcdesk.com` 搜索站、报告详情、账号、下载权限、管理后台、运营后台、静态构建、Cloudflare Worker、R2/Supabase 数据和发布链路。

维护规则：凡涉及账号、登录、权限、老客户兼容、联系方式、Worker API、数据存储或部署链路的改动，必须在同一个 PR 或提交中同步更新本文件。文档只记录 secret/var 名称，不记录值。

## 1. 当前业务边界

1. KCdesk 不提供在线支付，不创建 Paddle checkout，也不接收有效支付 webhook。
2. KCdesk 与 Vid2PPT 的新支付赠送、自动发放和兑换关联已下架。
3. 网站前台、账号弹窗、报告页和后台展示不出现 Vid2PPT、NOVA、ATLAS、赞助链接或兑换码文案。
4. 新开通或调整下载权限只能人工联系：浏览器首选语言为中文时显示微信 `MacroGate`；其他语言显示邮箱 `econ.scroll@gmail.com`。
5. 历史上已经写入的有效权益继续按自己的范围、期限和下载额度参与鉴权，直到到期或管理员调整；此次下架不清空或缩短老客户权益。
6. Worker 内旧接收/兑换处理结构保留在 `KCDESK_VID2PPT_LINK_ENABLED` flag 后，生产默认未配置，即为关闭并返回 410。恢复前必须同步恢复前端说明并完成受控验证。

## 2. 总览

```mermaid
flowchart LR
  U["用户浏览器"] --> SITE["kcdesk.com 静态页面"]
...
| 路径 | 用途 |
| --- | --- |
| `kc_desk_notes/site_src/` | 前端源文件、账号弹窗、报告页和联系方式。 |
| `kc_desk_notes/site_src/assets/contact.js` | 依据浏览器首选语言选择微信或邮箱。 |
| `kc_desk_notes/site_src/assets/app.js` | 搜索、详情、账号、权限、管理/运营后台 UI。 |
| `workers/kc-desk-notes-worker/src/index.js` | 鉴权、下载、权限融合、管理 API、缓存与埋点。 |
| `kc_desk_notes/data/catalog.json` | 报告 catalog。 |
| `kc_desk_notes/data/history_catalog.json` | 历史 Text only 报告元数据。 |
| `kc_desk_notes/password_rules.json` | 通用密码 hash 与备用分组规则。 |
| `scripts/build_kc_desk_notes_site.py` | 构建 GitHub Pages artifact。 |
| `.github/workflows/kc-desk-notes-pages.yml` | 主构建、Pages 发布和 Worker 部署。 |
| `.github/workflows/kcdesk-worker-emergency-deploy.yml` | Worker 紧急发布。 |
| `.github/workflows/kcdesk-pages-emergency-deploy.yml` | Pages 紧急发布。 |
...
4. `scripts/build_kc_desk_notes_site.py` 生成静态 artifact。
5. 发布 GitHub Pages。
6. 使用 Wrangler 发布 Cloudflare Worker。

账号/权限改动必须同时发布 Worker 与 Pages。发布后检查登录、老客户下载、管理员权限保存、`liuxin` 运营后台、联系语言和已关闭接口。
...
- 中文浏览器：`如需开通或调整下载权限，请联系微信 MacroGate。`
- 非中文浏览器：`To activate or update download access, email econ.scroll@gmail.com.`
...
- `twotigers` 是 super 身份；管理后台按钮只对 super 显示。
- `liuxin` 的 operator 身份必须持续显示“运营后台”，不得被 super-only 显示逻辑覆盖。
- session token 由 Worker 服务端签名；前端隐藏按钮不能替代 API 鉴权。
- Supabase 是账号主库；配置暂不可用时，Worker 使用 R2 JSON 兼容路径。
- 老账号来源为空或 legacy 标识时，仍按 KCdesk 老账号处理，不改变登录方式。
...
| `/paddle-config` | GET | 410；KCdesk 不提供 checkout。 |
| `/paddle-webhook` | POST | 410；KCdesk 不接收支付 webhook。 |
...
生产环境不得设置 `KCDESK_VID2PPT_LINK_ENABLED=true`。需要恢复时必须先更新本文档、测试和所有用户可见说明。
...
- `zh`、`zh-CN`、`zh-TW` 等中文首选语言显示微信 `MacroGate`。
- 其它语言显示邮箱 `econ.scroll@gmail.com`。
...
| `KCDESK_VID2PPT_LINK_ENABLED` | var | 旧跨站处理总开关；生产必须保持关闭/未配置。 |
...
2. 运行 `test_kcdesk_contact_locale.js`。
3. 运行 `test_kcdesk_legacy_access_compat.js`。
...
3. 多来源融合按范围与期限分别生效。
4. `liuxin` 可见并可打开运营后台。
5. `twotigers` 可保存和读取用户权限。
6. 中文浏览器显示 MacroGate，非中文浏览器显示邮箱。
