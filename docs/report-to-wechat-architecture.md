...
3. 用 DeepSeek 生成短版中文文章、标题和 编辑评论。
4. 用模型约束和确定性规则把公众号标题统一改成中性事实表达。
5. 生成 Portal 中文汇编 PDF 和公众号 HTML。
...
flowchart LR
  ...
  F --> G["Portal translated artifacts"]
...
| `scripts/build_portal_translated_reports.py` | 精译、文章化、标题生成与中性化和 Portal PDF |
| `scripts/commit_output_dir.sh` | 并发环境中的 fetch/reset/add/commit/push 重试和关键交接校验 |
| `scripts/push_portal_translated_to_wechat_drafts.py` | HTML、三张正文图、慢速图片上传、草稿创建与回读校验 |
| `scripts/push_xhs_notes_to_wechat_drafts.py` | 投行短版文章直传；与 Portal 精译上传共用图片、文章、草稿和回读等待节奏 |
...
- `portal_translated_reports/<source>/<date>/`：公众号上游和每日汇编输入。
...
- 生成端、Portal 精译上传端和 XHS 直传端共用同一中性化函数；上传前会重写正文第一个 H1，并逐一检查其余 H2/H3，防止外层标题已降温而正文可见小标题仍保留评价性或敏感措辞。标题改写永远不能成为删除文章、跳过报告或减少草稿数量的理由。
- 个股报告不得出现目标价、评级、买入、卖出或推荐措辞。
- 正文不放中间 CTA；只保留 编辑评论和最结尾统一文字 `更新信息参见portal.example.invalid`。
...
- Portal Suite operations alert 记录保存在 R2 `_ops/alerts/`；同一 GitHub run 只确认发送一次。
